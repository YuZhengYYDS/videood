...
class ConfigBuilder(object):
    ...
    def get_model(self, model_params = None):
        ...
        if type == 'autoencoder_kl_gan_model':
    ...
    def get_dataset(self, dataset_params = None, split = 'train'):
        ...
        if type(dataset_params) == dict:
            ...
            if dataset_type == 'hmdb':
                from datasets.hmdb import HMDBFrame
                # Remove the key 'type' so that HMDBFrame does not receive an unexpected argument.
                dataset_params.pop('type', None)

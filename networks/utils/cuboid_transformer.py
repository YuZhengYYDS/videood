...
#     import sys
#     sys.path.append('/mnt/cache/gongjunchao/workdir/empty/earth-forecasting-transformer/src/earthformer/cuboid_transformer')
...
if __name__ == "__main__":
    ...
    input_data = torch.randn((b, inp_length, h, w, c)).to(device) #torch.randn((b, inp_length, c, h, w)).to(device)
    target = torch.randn((b, pred_length, h, w, c)).to(device) #

    print('load yaml from config')
    import yaml
    from omegaconf import OmegaConf
    cfg_path = '/mnt/cache/gongjunchao/workdir/empty/earth-forecasting-transformer/scripts/cuboid_transformer/sevir/cfg_sevir.yaml'
    with open(cfg_path, 'r') as cfg_file:
      cfg_params = yaml.load(cfg_file, Loader = yaml.FullLoader)
    oc_from_file = OmegaConf.load(open(cfg_path, "r"))
    ## merge cfg_params ##
    def get_model_config():
        cfg = OmegaConf.create()

        #cfg.base_units = 64 #128
        ...
        #cfg.initial_downsample_stack_conv_dim_list = [4, 16, cfg.base_units] #[16, 64, 128]
        ...
        return cfg
    dynamic_oc = OmegaConf.create()
    dynamic_oc.model = get_model_config()

    oc_cfg = OmegaConf.merge(oc_from_file, dynamic_oc)
    
    # backbone_kwargs = model_cfg['model']
    model_cfg = oc_cfg['model']
...
# srun -p ai4earth --kill-on-bad-exit=1 --quotatype=auto --gres=gpu:1 python -u cuboid_transformer.py #
...
def get_data_loader_length(data_loader):
    if torch.distributed.is_initialized() and get_tensor_model_parallel_world_size() > 1:
        if data_loader is not None:
            max_step = torch.tensor(len(data_loader))
        else:
            max_step = None
